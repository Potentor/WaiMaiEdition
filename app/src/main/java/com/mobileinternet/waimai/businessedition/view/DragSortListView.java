...
public class DragSortListView extends ListView {
    ...
    private boolean handleDownEvent(MotionEvent ev) {
      ...
      //  View dragHandler = underFingerItemView.findViewById(R.id.tv_drag_handler);
        View dragHandler = underFingerItemView.findViewById(R.id.item_dish_iv_move);
        if (dragHandler == null) {
...
//        underFingerItemView.getHitRect(underFingerItemViewDrawingRect); the same as up one, the same coordinate , the same size
        ...
        ((ISortFace) getAdapter()).setDragSrcPosition(srcPosition);
...
//        reorderListView(ev);
        ...
        ((ISortFace) getAdapter()).setDragSrcPosition(currPosition);
        ((ISortFace) getAdapter()).moveItem(srcPosition, currPosition);
...
//        reorderListView(ev);
        ...
        ((ISortFace) getAdapter()).setDragSrcPosition(-1);

        //move item even if srcPosition equals with dstPosition, because we want to refresh ListView
        ((ISortFace) getAdapter()).moveItem(srcPosition, dstPosition);
        srcPosition = -1;
    }
